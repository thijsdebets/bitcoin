...
if [ "$bitcoin" != "" ] && [ "${bitcoin%.*}" -gt "100" ] ; then
	...
	curl "http://127.0.0.1:8080/json.htm?type=command&param=udevice&idx=300&nvalue=0&svalue=$bitcoin;0"
	curl "http://127.0.0.1:8080/json.htm?type=command&param=udevice&idx=309&nvalue=0&svalue=$wallet;0"
fi
